...
// const db = require('../util/moduleKeyValueStore');
// const hash = require('../util/createHashIdentity')
...
// const CONFIG_CHAINCODE_ID_BY_PROXY = config.chaincodeId2;
// const CONFIG_CHANNEL_NAME_BY_PROXY = config.channel2;
const CONFIG_ORGANIZATION_ID = config.orgId
...
function init() {
    ...
    logger.debug(`${functionName} => CHAINCODE${CONFIG_CHAINCODE_ID} ,CHANNEL : ${CONFIG_CANNEL_NAME}`)

    logger.info(`${functionName} Initialising`);
    // db.CreateDB().then((error) => {
    //     if (error != null) {
    //         logger.error(`CreateDatabase error : ${error}`)
    //     }
    // });
    return (() => {
        ...
        _channel = _fabricClient.getChannel(CONFIG_CANNEL_NAME, false)
        if (!_channel) {
            _channel = _fabricClient.newChannel(CONFIG_CANNEL_NAME);
        }
        // _channel_priv = _fabricClient.getChannel(CONFIG_CHANNEL_NAME_BY_PROXY, false)
        // if (!_channel_priv) {
        //     _channel_priv = _fabricClient.newChannel(CONFIG_CHANNEL_NAME_BY_PROXY);
        // }
        CONFIG_PEERS_ARRAY.forEach(function (peer) {
            ...
            if (peer.cert !== "" && peer.cert) {
                // peerConf['ssl-target-name-override'] = peer.grpcOptions['ssl-target-name-override'];
                peerConf.pem = peer.cert;
            }

            let peerObj = _fabricClient.newPeer(peer.requestURL, peerConf);
            _channel.addPeer(peerObj);
            // _channel_priv.addPeer(peerObj);
        ...
        CONFIG_ORDERERS_ARRAY.forEach(function (orderer) {
            ...
            if (orderer.cert !== "" && orderer.cert) {
                // ordererConf['ssl-target-name-override'] = orderer.grpcOptions['ssl-target-name-override'];
            ...
            // _channel_priv.addOrderer(ordererObj);
        }, this);
        ...
        // db.checkKeyExists("admin").then((bool) => {
        //     if (!bool) {
        ...
        //             password: password
        //         }
        //         logger.info(`Store user : admin `);
        //         db.putStore("admin", JSON.stringify(data))
...
// -------------   Private functions ------------------------
function getConfigChannel(endpoint) {
    if (endpoint == "public") {
        return CONFIG_CANNEL_NAME
    }
    else if (endpoint == "private") {
        return CONFIG_CHANNEL_NAME_BY_PROXY
    }
    else {
        // throw error
    }
}
function getConfigChaincodeID(endpoint) {
    if (endpoint == "public") {
        return CONFIG_CHAINCODE_ID
    }
    else if (endpoint == "private") {
        return CONFIG_CHAINCODE_ID_BY_PROXY
    }
    else {
        // throw error
    }
...
// Invoke and query
// -------------------
function invoke(enrollmentId, fcn, args, endpoint, isQuery) {
    ...
    return new Promise((resolve, reject) => {
        ...
        (() => {
            ...
            .then((user) => {
                ...
                _channel = _fabricClient.getChannel(CONFIG_CANNEL_NAME, true)
                if (!_channel) {
                    _channel = _fabricClient.newChannel(CONFIG_CANNEL_NAME);
                }
            ...
            .then((results) => {
                ...
                if (!isQuery) {
                    ...
                    //console.log("rerererer"+JSON.stringify(results[0][0].response.message))
                    if (all_good) {
                        ...
                        // proposalResponses[0].response.status = 400
                        // proposalResponses[1].response.status = 400
                        //logger.debug(`pp1 ${JSON.stringify(proposalResponses[0].response.status )}`)
                        //logger.debug(`pp1 ${JSON.stringify(proposalResponses[1].response.status )}`)
                        // logger.debug(`pp1 ${JSON.stringify(proposalResponses[1].endorsement )}`)
                        // logger.debug(`pp1 ${JSON.stringify(proposalResponses[1].endorsement )}`)
                        // logger.debug(`pp1 ${JSON.stringify(proposalResponses[0].endorsement )}`)
                        // logger.debug(`pp1 ${JSON.stringify(proposalResponses[1].endorsement )}`)
                        const request = {
                        ...
                        return Promise.all([sendPromise].concat()).then((results) => {
                        ...
                        logger.error(`${functionName} Failed to send Proposal or receive valid response. Response null or status is not 200. exiting...`);
                        // logger.error("~!fdsnkfnsdjfnsjdf@"+JSON.stringify(results[0][0].response.message))
                    ...
                    for (let i = 0; i < proposalResponses.length; i++) {
                        ...
                        if (response instanceof Error) {
                            ...
                            error = response.message.split("message:");
                    ...
                    if (results.length) {
                        ...
                        throw new Error(`All peers returned (${error[1].trim()}`);
                        reject(`${functionName} All peers returned (see above)`);
                    }
                ...
                if (!isQuery) {
                    if (response.status === 'SUCCESS') {
                        ...
                        throw new Error(`${functionName} Failed to order the transaction. Error code: ${response.status}  ${response.message}`);
                        reject(`failed to order the transaction. Error code: ${response.message}`);
                    }
                } else {
...
function query(enrollmentId, fcn, args, endpoint) {
    //TODO: Delete this deprecated attr

    return invoke(enrollmentId, fcn, args, endpoint, true);

} // End of query() function

function registerAndEnroll(enrollmentId, affiliation, attrs, role) {
    let functionName = '[Blockchain.service.registerAndEnroll()]';

    const registrationDetails = {
        enrollmentID: enrollmentId,
        enrollmentSecret: null,
        role: role || 'client',
        affiliation: affiliation || CONFIG_ORGANIZATION_ID,
        maxEnrollments: 1,
        attributes: attrs || [{
            name: 'devorgId',
            value: 'test'
    ...
    return new Promise((resolve, reject) => {
        ...
        (function () {
            ...
            .then(() => {
                logger.debug(`${functionName} Checking if user ${enrollmentId} already enrolled.`);
                return _fabricClient.getUserContext(enrollmentId, true);
            }).then((user) => {
                if (user !== undefined && user !== null && user.isEnrolled() !== false) {
                    throw new Error(`${functionName} Error: User ${enrollmentId} is already enrolled.`);
                    reject(`user ${enrollmentId} is already enrolled.`);
                }
                //_registrarUser=user
                return _fabricCAService.register(registrationDetails, _registrarUser);
            }).then((enrollmentSecret) => {
                logger.debug(`${functionName} Got enrollment secret for user ${enrollmentId}: ${JSON.stringify(enrollmentSecret)}`);

                let enrollmentDetails = {
                    enrollmentID: enrollmentId,
                    enrollmentSecret: enrollmentSecret
                }
                return _fabricCAService.enroll(enrollmentDetails);
            }).then((enrollment) => {
                logger.debug(`${functionName} Got enrollment response for ${enrollmentId}. Creating user identity.`);

                user = new User(enrollmentId);
                user.setCryptoSuite(_cryptoSuite);
                return user.setEnrollment(enrollment.key, enrollment.certificate, CONFIG_MSPID);
            }).then(() => {
                logger.debug(`${functionName} Enrollment object for ${enrollmentId} is set.`);
                return _keyValueStore.setValue(user.getName(), user.toString());
            }).then(() => {
                logger.debug(`${functionName} Resolving promise with registered enrolled user object for ${enrollmentId}.`);
...
} //End of registerAndEnroll() function
...
module.exports = {
    ...
    invoke: invoke,
    query: query,
    registerAndEnroll: registerAndEnroll,
}